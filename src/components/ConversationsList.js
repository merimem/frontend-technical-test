...
function ConversationsList(props) {
    ...
    const { conversations, loading, error } = useGetConversations(id);
    let { allAvatars } = props
    if( ! conversations)
        return null;
                    ...
                    {conversations?
                         conversations.map(conv =>{
                            ...
                            return <Conversation key={conv.id}
                                    name={getReceipientNickname(conv) }
...
const mapDispacthToProps = (dispatch) => {
  ...
  export default connect(mapStateToProps, mapDispacthToProps)(ConversationsList)